...
Такое расположение датацентров выбрано исходя из географического распределения трафика, а также карты подводных кабелей.
...
В качестве DNS балансировки будет использоваться Geo-based DNS, т. к. для отправки почты не требуется минимальная задержка.

### Anycast балансировка

Внутри регионов будет использоваться BGP Anycast для более точной настройки распределения нагрузки между датацентрами.
